...
For example, the popular FEniCS [TODO ref] and Firedrake [TODO ref] FEM packages for
...
\begin{itemize}
\item Multiple dispatch allows the cubature scheme to be selected at compile
time based on the dimensionality, the degree of the singularity, the degree of
...
\noindent
This syntax is ``compiled'' to a dense numerical representation that can
be consumed by standard solver libraries.
Although solving the problem can take a significant amount of time,
the time spent building the problem representation is often a bottleneck.
...
$L$ is the number of locations included in an area-coverage model.
}
}
\label{jumpperf}
\end{table}
