...
Date and time arithmetic is especially intricate and irregular.
...
% the same code from Haskell, Repa
Repa~\cite{Keller:2010rs}
\begin{verbatim}
...
# any kernel that decays as ~$r^p$~, p < 0
abstract APowerLaw{p} <: AbstractKernel

# ~$r^p$~ power law
type PowerLaw{p} <: APowerLaw{p}; end
\end{lstlisting}
\end{singlespace}
...
\begin{singlespace}
\begin{lstlisting}[language=julia]
type Helmholtz{k} <: APowerLaw{-1}; end  # ~$e^{ikr} / 4\pi r$~
...
type MFIE{k} <: AbstractKernel; end  # ~$(ikr - 1) * e^{ikr} / 4\pi r^3$~
...
case of any \texttt{APowerLaw}.
However, each kernel requires a large amount of up-front work that cannot be done
efficiently at run time.
Julia's staged functions solve this problem.
% TODO more

\begin{singlespace}
\begin{lstlisting}[language=julia]
@generated function call{p,P<:APowerLaw{p},n}(K::FirstIntegral{P,n},
                                              X::Real)
    quote
...
pseudocode for how this is used:

%  # n depends on complicated stuff
\begin{singlespace}
\begin{lstlisting}[language=julia]
# from paper figure 1
Fct = [FirstIntegral{K,i}() for i=1:n]
Pct = [some polynomials...]
for trianglepair in mesh
    i, j = indexes based on triangles
    if common(trianglepair)
        M[i,j] = integrate(y->sum([Fct[p](y)*Pct[p](y) for p=1:n]),
                           0, 1)
