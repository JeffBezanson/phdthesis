...
(who, of course, might be the same person as the programmer).
...
Notice that we have neither added restrictions, nor asked for
...
% three forms of ``gradualism'': adding methods, adding types to arguments,
% adding parameters to types.
...
languages.
But in this example still more can be done.
Notice that in the first step to objects we replaced one level of \texttt{if}
...
\hline
               & Domain &  Dynamic  & Spec. & ADTs & Pattern & S.T.S. & S.C. \\
\hline
\hline
...
\hline
Predicates     & $O(n m)$ & \chk    &       & \chk &  \small{1}  & ? & ? \\
\hline
Typeclasses    & $O(m)$   & \small{2}    &  \small{3}        & \chk &     & \chk & \chk \\
\hline
Term rewriting & $O(n m)$ & \chk    &       &      & \chk    &        &      \\
\hline
Julia          & $O(n m)$ & \chk    & \chk  & \chk &         &        &      \\
\hline
\end{tabular}
\end{center}
\begin{singlespace}
...
Table~\ref{table:dispatch} compares nine language features.
...
taken into account.
``ADTs'' refers to support for abstract data types.
...
Including more than one of these mechanisms makes a languge especially
complex and difficult to learn.


\section{Subtyping}

So far we have seen some reasons why dispatch contributes so significantly
...
The theory governing such questions is subtyping.

It turns out that a lot of relevant work on this has been done in the
context of type systems for XML.
